...
#@export
var toggle_mode := false setget set_toggle_mode
#@export
var button_pressed := false setget set_button_pressed
...
#@export
var group: TouchButtonGroup setget set_button_group
...
func _get_property_list():
	return [
		...
		{ name = "action_mode", type = TYPE_INT, hint = PROPERTY_HINT_ENUM, hint_string = "Button Press,ButtonRelease" },
		{ name = "passby_press", type = TYPE_BOOL },
		{ name = "pass_screen_drag", type = TYPE_BOOL },
		{ name = "group", type = TYPE_OBJECT, hint = PROPERTY_HINT_RESOURCE_TYPE, hint_string = "Resource" }
	]
...
func set_button_group(value):
	...
	if is_instance_valid(group):
		...
		group.disconnect("pressed", self, "_on_group_button_pressed")
	group = value
	if is_instance_valid(value):
		if !(self in group._buttons):
			group._buttons.push_back(self)
			if !group.is_connected("pressed", self, "_on_group_button_pressed"):
				group.connect("pressed", self, "_on_group_button_pressed")
	update_configuration_warning()
